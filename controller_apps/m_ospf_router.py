    ...
    def _update_routing_tbl(self, msg, header_list):
        ...
        for key, value in self.routing_tbl.items():
            if key == 'groups':
                continue
            for route in value:
                if route.gateway_ip == src_ip:
                    gateway_flg = True
                    if route.gateway_mac == src_mac:
                        continue
...
#                    continue
                    route.gateway_mac = src_mac
                    route.local_port = out_port
                    route.local_mac = dst_mac

                    cookie = self._id_to_cookie(REST_ROUTEID, route.route_id)
                    priority, log_msg = self._get_priority(PRIORITY_TYPE_ROUTE,
                                                       route=route)

                    group_id = int(str(self.routing_tbl['groups'][key])+'0'+str(self.vlan_id))
                    if len(value) == 1:
                        group_flg = True
                    # Here we had the flow rule doing routing directly.
                    # Now we're updating the group data with each route in the list as a bucket;
...
#                                                dst_mask=route.netmask,
#                                                dec_ttl=True)
                    self.logger.info('Changed %s flow [cookie=0x%x]', log_msg, cookie,
                                 extra=self.sw_id)
        return gateway_flg
...
# Changed to store a list of possible routes instead of a single one
class RoutingTable(dict):
    ...
    # Add the routing data to a list for each key
...
#            raise CommandFailure(msg=msg)
        ...
        group = self.setdefault(key,[])
        if len(group) == 0:
            #First route added for destination, create the group for it
            self['groups'][key] = self.group_id
            self.group_id += 1
        self[key].append(routing_data)
    ...
    # Delete only the route with the ID in the lists. If there are no more routes, then remove the key.
    def delete(self, route_id):
        for key, value in self.items():
            if type(value) is list:
                for route in value:
                    if route.route_id == route_id:
                        self[key].remove(route)
                        if len(value)==0:
                            del self[key]
                            del self['groups'][key]
                        return
    
    # Get a list of gateways from the table; now needs to search each list for each key.
    def get_gateways(self):
        gatewaylist = []
    
        for routelist in self.values():
            if type(routelist) is list:
                for route in routelist:
                    gatewaylist.append(route.gateway_ip)
...
#        return [routing_data.gateway_ip for routing_data in self.values()]
    ...
    # First approach: return the whole list instead of a single route. Advantages are we have no idea where this is called really and theres no much to change.
    # Works fine, but if one queries a mac address, return a single route (just for gateway id)
        ...
        if gw_mac is not None:
            ...
            for route in self.values():
                if type(route) is list:
                    if ipv4_apply_mask(dst_ip, route[0].netmask) == route[0].dst_ip:
                        # For longest match
                        if mask < route[0].netmask:
                            get_route = route
                            mask = route[0].netmask
...
class Route(object):
    def __init__(self, route_id, dst_ip, netmask, gateway_ip):
        ...
        # Added to the Route object to match the Group Tables needs.
        self.local_mac = None
...
class OfCtl_after_v1_2(OfCtl):
    ...
    # Add the flow in table 1 to resubmit to the group table
    def set_routing_group_flow(self, cookie, priority, group_id, dl_vlan=0,
                         nw_dst=0, dst_mask=32, dec_ttl=False):
        ofp = self.dp.ofproto
        ofp_parser = self.dp.ofproto_parser
        ...
        if dec_ttl:
            actions.append(ofp_parser.OFPActionDecNwTtl())
        actions.append(ofp_parser.OFPActionGroup(group_id = group_id))

        self.set_flow(cookie, priority, dl_type=dl_type, dl_vlan=dl_vlan,
                      nw_dst=nw_dst, dst_mask=dst_mask, actions=actions)
    
    # Update the group buckets
    def update_routing_group(self, group_id, group_flg, routelist):
        ofp = self.dp.ofproto
        ofp_parser = self.dp.ofproto_parser
        if group_flg:
            cmd = ofp.OFPGC_ADD
        else:
            cmd = ofp.OFPGC_MODIFY
        ...
        for route in routelist:
            actions = []
            actions.append(ofp_parser.OFPActionSetField(eth_src = route.local_mac))
            actions.append(ofp_parser.OFPActionSetField(eth_dst = route.gateway_mac))
            actions.append(ofp_parser.OFPActionOutput(route.local_port, 0))
            watch_port = route.local_port
            bucketlist.append(ofp_parser.OFPBucket(1, watch_port, watch_group, actions))

        m = ofp_parser.OFPGroupMod(self.dp, cmd, ofp.OFPGT_SELECT, group_id, bucketlist)
